...
//           latestMessage.receiverViewed === true
//         )
//           return;

//         // Inform the backend that the message has been read
//         const res = await authFetch(`/api/chat/message/read/${latestMessage.id}`, {
//           method: "PATCH",
//           headers: {
//             "Content-Type": "application/json",
//           },
...
export default function ChatScreen() {
  ...
  const { userId } = useAuth();
  const { messages, match, setMessages } = useChat(matchId);
  const authFetch = useAuthFetch();
  const flatListRef = useRef<FlatList>(null);
  const [isSending, setIsSending] = useState(false);
  const [showProfile, setShowProfile] = useState(false);
  ...
  const sendMessage = async (text: string) => {
    setIsSending(true);
    try {
      await authFetch("/api/chat/send-message", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ matchId, messageText: text }),
      });
    } catch (err) {
            ...
            renderItem={({ item }) => (
              ...
              >
                ...
                <View className="max-w-[80%]">
                  ...
                  {item.senderId === userId && (
                    <View className="flex-row items-center self-end mr-2">
                      <Text className="text-xs mr-2 text-gray-500">Sent</Text>
                      <Icon name="check-circle" size={20} color="#8fb3a1" />
                    </View>
                  )}
